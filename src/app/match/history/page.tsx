  ...
  const fetchMatches = async () => {
    try {
      ...
      const sortedData = [...data].sort((a, b) => {
        ...
        // 2. 日付が同じなら時間で比較（降順: 遅い時間の試合を上にする場合）
        // ※ 昇順（第1試合を上）にしたい場合は a.time.localeCompare(b.time) に変更してください
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans pb-24">
      ...
      <div className="relative h-64 bg-gradient-to-b from-blue-800 to-slate-900 flex flex-col items-center justify-center text-center px-4 overflow-hidden">
        ...
        <h1 className="text-5xl font-black italic tracking-tighter mb-2 z-10 drop-shadow-lg text-white">
          MATCH ARCHIVE
        </h1>
        ...
        <div className="space-y-6">
          {matches.length === 0 ? (
            <div className="text-center py-20 bg-slate-800/30 rounded-[2rem] border border-dashed border-slate-700">
              ...
              <Link href="/match/entry" className="text-blue-500 text-sm mt-4 inline-block font-bold">最初の試合を記録する →</Link>
            </div>
          ) : (
            matches.map((match) => {
              ...
              return (
                <div key={match.id} className="bg-white text-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl relative border border-white">
                  ...
                  <div className="absolute top-4 right-4 z-30 flex gap-2">
                    <Link href={`/match/entry?id=${match.id}`} className="bg-white/90 p-2 rounded-full shadow-md text-blue-600 hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center w-8 h-8">
                      <span className="text-xs">✎</span>
                    </Link>
                    <button onClick={() => handleDelete(match.id)} className="bg-white/90 p-2 rounded-full shadow-md text-red-600 hover:bg-red-600 hover:text-white transition-all flex items-center justify-center w-8 h-8">
                      <span className="text-xs">✕</span>
                    </button>
                  </div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex flex-col gap-1 mb-6 border-b border-slate-100 pb-4">
                      <div className="flex justify-between items-center">
                        <span className="text-[11px] font-black text-slate-400 tracking-tighter uppercase">
                          📅 {match.date} <span className="ml-1 text-slate-300">/</span> <span className="text-blue-500">{match.time || "--:--"}</span>
                        </span>
                        {match.matchStep && (
                      ...
                      <h3 className="text-xs font-black text-slate-800 flex items-center gap-1 mt-1">
                    ...
                    <div className="flex items-center justify-between gap-1 w-full px-2">
                      <div className="flex-1 text-center">
                        <p className="text-[9px] font-black text-slate-400 mb-1 tracking-tighter uppercase">Our Team</p>
                      ...
                      <div className="flex-1 text-center">
                        <p className="text-[9px] font-black text-slate-400 mb-1 tracking-tighter uppercase">Opponent</p>
