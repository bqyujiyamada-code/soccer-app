...
// --- 設定値・ロジック ---
const UNIT_PRICES = [
  { upTo: 1000, price: 0.3 }, // 累計1000回までは0.3円
  { upTo: 5000, price: 0.4 }, // 5000回までは0.4円
  { upTo: 999999, price: 0.5 }, // それ以降は0.5円
];
...
export default function LiftingEntry() {
  ...
  const calculateReward = () => {
    ...
    if (!n || n <= 0) return 0;
    let reward = 10; // 最低保証
    reward += n * currentLevelPrice; // 基本給
    if (isBestUpdate) reward += 20; // ベスト更新
    if ((userStats.combo + 1) % 5 === 0) reward *= 1.5; // 5日連続コンボ
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-4 flex items-center justify-center font-sans text-slate-100">
      <div className="w-full max-w-md bg-slate-800 rounded-[2.5rem] shadow-2xl p-6 border-b-8 border-green-600 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input 
              ...
              className="w-full bg-slate-700 border-2 border-slate-600 rounded-2xl p-4 font-bold text-white outline-none focus:border-green-500 transition-all"
            />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                className={`w-full bg-slate-700 border-4 ${isBestUpdate ? 'border-orange-500 animate-pulse' : 'border-slate-600'} rounded-3xl p-8 text-6xl text-center font-black outline-none focus:border-green-500 transition-all text-white`}
                required 
              />
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-slate-700 border-2 border-slate-600 rounded-2xl p-4 text-white focus:border-green-500 outline-none" 
